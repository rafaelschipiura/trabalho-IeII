...
RNGversion("4.1.2")
set.seed(1752031105)
library(formatR, ggplot2, reshape2)
knitr::opts_chunk$set(tidy.opts = list(width.cutoff = 50), tidy = TRUE)
knitr::opts_chunk$set(dev = 'pdf', fig.width=6, fig.height=5, fig.align='center')
```
```{r, echo=FALSE}
...
main <- function(){
   ...
   for(p in 1:primeira){
	    ...
	    #cat(knitr::kable(matriz, 'latex'))
            if (i == 'Trade-Off Beta'){
	       plot <- melt(matriz)
	       plot$n <- sprintf("%04d",plot$n)
               print(ggplot(data=plot,aes(x=n, y=d)) + geom_tile(aes(fill = value), color = "white", lwd = 0.1, linetype = 1) + coord_fixed() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + guides(fill = guide_colourbar(barwidth = 0.5, barheight = 20, title = "Poder")) + xlab("Tamanho da Amostra") + ylab("Significância") + geom_text(aes(label = sprintf('%.02f%%', 100*value)), color = "white", size = 4))
	    } else {
