...
main <- function(){
   ...
   for(n in 1:length(tamanhos_m)){
      resul_amostras[[n]] <- simula_s(tamanhos_m[n], significancias_m, delta_m)
   }
   ...
   for(s in 1:length(significancias_m)){
      resul_signif[[s]] <- simula_n(tamanhos_m, significancias_m[s], delta_m)
   }
...
IC <- function(amostra_i, sig_i){
   ic <- t.test(amostra_i, conf.level=(1-sig_i))$conf.int
   return(ic)
}

erros <- function(mu_e, intervalo, delta){
   errado.tipoI <- !(intervalo[1] < mu_e & mu_e < intervalo[2])
   errado.tipoII <- !((abs(mu_e) > delta) & ((intervalo[1] > 0) | (intervalo[2] < 0)))
...
varia_n <- function(n_n, sig_n, delta_n){
   ...
   for(n_j in 1:length(n_n)){
      resultado[n_j,] <- erros(mu_n, IC( sample(amostra_n, n_n[n_j]), sig_n), delta_n)
...
varia_s <- function(n_s, sig_s, delta_s){
   ...
   for(s_j in 1:length(sig_s)){
      resultado[s_j,] <- erros(mu_s, IC(amostra_s, sig_s[s_j]), delta_s)
   }
   return(resultado)
}
...
```{r}
simula_n <- function(n_n, sig_n, delta_n){
   D3 <- array(dim=c(length(n_n), 2, 1000))
   for(i in 1:1000){
	   D3[,,i] <- varia_n(n_n, sig_n, delta_n)
   }
...
simula_s <- function(n_s, sig_s, delta_s){
   D3 <- array(dim=c(length(sig_s), 2, 1000))
   for(i in 1:1000){
	   D3[,,i] <- varia_s(n_s, sig_s, delta_s)
   }
   result <- apply(D3, MARGIN=c(1,2), FUN=mean)
   colnames(result) <- c("Tipo I", "Tipo II")
   rownames(result) <- paste0(100*sig_s, "%") 
   return(result)
}
```
