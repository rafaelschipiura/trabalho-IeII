...
trabalho <- function(tamanhos_m, significancias_m, distro_m){
   ...
   for(n in 1:length(tamanhos_m)){
      resul_amostras[[n]] <- simula_s(tamanhos_m[n], significancias_m, distro=distro_m, pontos_s=pontos)
   }
   ...
   for(s in 1:length(significancias_m)){
      resul_signif[[s]] <- simula_n(tamanhos_m, significancias_m[s], distro=distro_m, pontos_n=pontos)
   }
...
```{r}
simula_n <- function(n_n, sig_n, distro, pontos_n){
   D3 <- array(dim=c(pontos_n, 2, 1000))
   for(i in 1:1000){
...
simula_s <- function(n_s, sig_s, distro, pontos_s){
   D3 <- array(dim=c(pontos_s, 2, 1000))
   for(i in 1:1000){
