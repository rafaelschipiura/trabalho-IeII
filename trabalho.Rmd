...
Avalia $\beta$ sob $H_a$:
 ...
 - $y_A \sim N(\mu_0, \sigma_A^2)$.

 - $y_B \sim N(\mu_0 + \delta, \sigma_B^2)$.
...
```{r, eval=FALSE}
teste <- function(amostra1, amostra2, sig){t.test(amostra1, amostra2)$p.value < sig}
amostra <- function(n, delta){rnorm(n, mean=20+delta, sd=2)}
alpha <- 0.05;delta <- 0.1;ene <- 30;size <- 3000 
matriz[n][delta][alpha] <- mean(replicate(size, teste(amostra(ene, 0), amostra(ene, delta),  alpha)))
