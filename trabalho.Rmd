...
main <- function(){
   ...
   delta_m <- 0.2
   #exibe(simula(tamanhos_m, significancias_m, delta_m), tamanhos_m)
   #knitr::kable(varia_n(tamanhos_m, significancias_m[1], delta_m))
...
```{r}
cobertura <- function(n_c, sig, delta_c){
   mu_c <- runif(1, -1, 1) 
   ...
   errado.tipoII <- abs(mu_c) > delta_c & IC_t.test[1] < 0 & 0 < IC_t.test[2] 
...
```{r}
simula <- function(tamanhos, significancias, delta){
   ...
   for(n in tamanhos){
      ...
      for(a in 1:length(significancias)){
         ...
         resultado[[as.character(n)]][a,] <- rowMeans(replicate(1000, cobertura(n, alpha, delta)))
      }
   }
   return(resultado)
}
...
```{r, results='asis'}
exibe <- function(resultado, tamanhos){
   for(tam in tamanhos){
      cat("\n## Amostra n = ", tam, "\n\n")
