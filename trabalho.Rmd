...
## Varia tamanho da amostra, significância fixa
...
varia_n <- function(n_n, sig_n, tipoII=FALSE, distro_n, param_n = 20, pontos){
   ...
   resultado <- matrix(FALSE, nrow=pontos, ncol=1) 
   for(n_j in 1:pontos){
      resultado[n_j, 1] <- teste( sample(amostra_n, n10[n_j]), sample(amostra_m, n10[n_j]), sig_n)
...
varia_s <- function(n_s, sig_s, tipoII=FALSE, distro_s, param_s = 20, pontos){
   ...
   resultado <- matrix(FALSE, nrow=pontos, ncol=1) 
   for(s_j in 1:pontos){
      resultado[s_j, 1] <- teste(amostra_s, amostra_r, s10[s_j])
   }
   return(resultado)
}
